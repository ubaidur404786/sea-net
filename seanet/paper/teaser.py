...
def _prepare_models(model_names: List[str], dataset: str, device, smoke: bool, seed: int,
    ...
    for i, name in enumerate(model_names):
        ...
        entries.append({
            ...
            "display": config_base,                          # the label shown on the row
            "model": model,
            "test_ds": test_ds,
...
def _draw(entries: List[Dict], idx: int, dataset: str, out_dir: str, smoke: bool) -> str:
    ...
    n = len(entries)
    fig = plt.figure(figsize=(11.0, 2.7 * n + 1.4))
    gs = fig.add_gridspec(n, 2, width_ratios=[1.0, 3.0], hspace=0.75, wspace=0.22)
    ...
    for i, (e, ex) in enumerate(zip(entries, explanations)):
        ...
        for c in range(n_clz):
            if probs[c] > 0.03:
                axL.text(c, probs[c] + 0.02, f"{probs[c]:.2f}", ha="center", va="bottom", fontsize=7)
        axL.set_ylim(0, 1.12)
        axL.set_xlabel("class", fontsize=8)
        axL.set_ylabel("prob.", fontsize=8)
        axL.tick_params(labelsize=7)
        if i == 0:
            axL.set_title("prediction", fontsize=9)
        ...
        axR.set_xlabel("timestep", fontsize=8)
        ...
        axR.set_title(banner, loc="left", fontsize=10.5,
    ...
    if scatter_handle is not None:
        cbar = fig.colorbar(scatter_handle, ax=right_axes, fraction=0.025, pad=0.02)
        cbar.set_label("importance to predicted class\n(red = supports, blue = against)", fontsize=8)
    ...
    fig.suptitle(f"{dataset}  #{idx}  —  true class {true_clz}: same input, {n} models "
                 f"—  {ratio:.0f}× fewer parameters, and it still explains itself",
