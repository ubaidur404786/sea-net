...
# the MILLET baseline MODELS (config names) - everything else in a cross-model comparison is one of our
# new pooling models, drawn in orange so it stands out.
BASELINE_MODELS = {"seanet", "seanet_conjunctive"}
...
def plot_model_comparison(cross: pd.DataFrame, figdir: str = FIGURES_DIR) -> List[str]:
    """
    Draw the cross-model comparison from compare_models(): mean test accuracy and mean AOPCR per model
    (over the UCR datasets), best first. Our new pooling models are drawn in orange so they stand out
    from the MILLET baselines (seanet = additive, seanet_conjunctive).
    ...
    if cross.empty or "model" not in cross.columns:
        return paths
    fig, ax = plt.subplots(1, 2, figsize=(12, 5))
    for a, col, title in [(ax[0], "mean_acc", "Mean test accuracy (UCR)"),
                          (ax[1], "mean_aopcr", "Mean AOPCR (interpretability)")]:
        s = cross.dropna(subset=[col]).sort_values(col, ascending=False)
        colors = ["steelblue" if m in BASELINE_MODELS else "darkorange" for m in s["model"]]
        bars = a.bar(range(len(s)), s[col].values, color=colors)
        a.set_xticks(range(len(s))); a.set_xticklabels(s["model"], rotation=30, ha="right")
        a.set_title(title); a.set_ylabel(col)
        for b, v in zip(bars, s[col].values):                   # write the value on top of each bar
            if pd.notna(v):
                a.text(b.get_x() + b.get_width() / 2, v, f"{v:.3f}", ha="center", va="bottom", fontsize=8)
    fig.suptitle("Pooling model comparison (mean over UCR datasets)   [orange = our new heads]")
    fig.tight_layout()
    p = os.path.join(figdir, "model_comparison.png"); fig.savefig(p, dpi=120); plt.close(fig); paths.append(p)
    return paths
...
def generate_report(figdir: str = FIGURES_DIR, verbose: bool = True) -> Dict:
    ...
    # optional: the cross-model comparison, only if more than one model has been swept (own folders).
    # compare_models() also refreshes each model's comparison_vs_millet.csv and model_comparison.csv.
    cross = R.compare_models(verbose=False)
    if len(cross) > 1:
        figures += plot_model_comparison(cross, figdir)
    ...
    if verbose:
        ...
        if len(cross) > 1:                                      # cross-model (full sweep) ranking, best first
            print("\n  full-sweep model comparison (mean UCR test_acc | AOPCR):")
            for _, r in cross.iterrows():
                mark = "" if r["model"] in BASELINE_MODELS else "  <- new"
                print(f"    {r['model']:22s} acc {r['mean_acc']:.4f}  AOPCR {r['mean_aopcr']:7.3f}"
                      f"  W/T/L {r['acc_win_tie_loss']}{mark}")
