...
It reads every model's results/top_results/SEA_NET/<model>/results.csv (through the leaderboard) and writes
results/top_results/analysis/ - the cross-model comparisons that answer the questions the project exists to
answer:
...
----------------------------------------------
    python main.py leaderboard          rebuilds results/top_results/SEA_NET/leaderboard.csv
...
def generate(refresh: bool = False, verbose: bool = True) -> Dict:
    ...
    refresh : True rebuilds the leaderboard from every model's results.csv first (slower, always
              current); False reuses results/top_results/SEA_NET/leaderboard.csv.
...
def write_index(figures: List[Dict], tables: List[Dict]) -> str:
    """
    Write results/top_results/analysis/INDEX.md - one page that says what every figure and table is.

    Without it, results/top_results/analysis/ is thirty PNGs with cryptic names. With it, you can read down
    ...
    lines = ["# Analysis output", "",
             ...
             "the next run overwrites it. The numbers come from `results/top_results/SEA_NET/*/results.csv`",
             "via `results/top_results/SEA_NET/leaderboard.csv`.", "",
             "## Figures", ""]
    for section in SECTION_ORDER:
        ...
        for entry in items:
            ...
            rel = rel.split("results/top_results/analysis/")[-1]
