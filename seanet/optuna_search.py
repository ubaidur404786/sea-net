"""
seanet/optuna_search.py - Optuna hyperparameter search (kept as close to the official tutorial as possible).
...
Related files:
OPTUNA IS OPTIONAL AND IT DOES NOT HAVE ITS OWN TRAINING CODE.
    A trial calls seanet.training.fit_model() - the exact same function `python main.py single`
    calls - with a config copy in which the sampled values have been substituted. So:

        Optuna -> the ONE training pipeline -> evaluation -> MLflow

    If you never run `python main.py optuna`, nothing in this file is imported and the rest of
    the project behaves identically. Optuna is not even a required dependency.

Related files:
    - configs/models/**/<model>.yaml -> the "optuna" block (settings + search_space) AND the records block.
    - configs/main.yaml   -> project-wide optuna defaults (n_trials, sampler, pruner).
    - seanet/training.py  -> fit_model() (one trial = one fit) and _train_kwargs_from_config().
    - seanet/metrics.py   -> the accuracy/loss a trial is scored with.
...
from seanet.metrics import classification_metrics as safe_evaluate
from seanet.models import num_params
from seanet.training import fit_model, _train_kwargs_from_config
from seanet.utils import get_device
...
def run_optuna(cfg, dataset=None, device=None, smoke: bool = False, verbose: bool = True):
    ...
    if ocfg is None or not getattr(ocfg, "enabled", False):
        ...
        return None
    # project-wide defaults from configs/main.yaml, used for anything the model block leaves out
    defaults = getattr(cfg, "optuna", None)

    def setting(key, fallback):
        """Read one optuna setting: the model block wins, then main.yaml, then the fallback."""
        if hasattr(ocfg, key):
            return getattr(ocfg, key)
        return getattr(defaults, key, fallback) if defaults is not None else fallback
    try:
        import optuna
    except ImportError:
    ...
    n_trials = 2 if smoke else int(setting("n_trials", 20))
    timeout = setting("timeout", None)
    ...
    print(f"trials={n_trials} sampler={setting('sampler', 'tpe')} "
          f"pruner={setting('pruner', 'median')} | tuning: {[p for p, _ in space]}")
    ...
    study = optuna.create_study(
        ...
        sampler=_build_sampler(setting("sampler", "tpe"), seed),
        pruner=_build_pruner(setting("pruner", "median")),
    )
    ...
    if smoke:
        ...
        return study
    if not setting("save_best_parameters", True):
        return study
