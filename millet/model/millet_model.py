    ...
    def evaluate_interpretability(
        ...
        with torch.no_grad():
            for batch in custom_tqdm(dataloader, leave=False):
                ...
                # SEA-Net fix (see README "Adjustments to millet"): mil_collate_fn ALWAYS puts an
                # "instance_targets" key in the batch (value None for datasets without instance
                # labels, e.g. every UCR dataset), so the original `if "instance_targets" in batch`
                # was always True and then indexed None -> crash. Check the value instead.
                if batch.get("instance_targets") is not None:
