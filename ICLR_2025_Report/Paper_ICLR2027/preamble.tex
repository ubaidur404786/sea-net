...
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{longtable}   % only for the 72-row appendix leaderboard
\usepackage{subcaption}
\usepackage{xcolor}
\usepackage{microtype}

% TikZ is only used for the architecture figure. Everything else is an
% external PDF produced by the experiment code.
\usepackage{tikz}
...
\newcommand{\ucr}{UCR}
\newcommand{\ndcg}{NDCG@$n$}
\newcommand{\seatiny}{\seanet{}-B}          % the headline 41 K model (bottleneck + Top-k)
...
% Slightly tighter floats, again for the page budget. The style file
% itself is never touched -- only these LaTeX lengths.
