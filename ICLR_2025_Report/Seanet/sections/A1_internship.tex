...
sites with the OAR job scheduler. The two environments are detailed in \Cref{tab:environment}.
...
O5. Build a reproducible benchmark
  ...
  & \textbf{Reached.} Any encoder/pooling pair trains from a single YAML file, and every figure and
    table here is regenerated by one command (\Cref{app:commands}). \\
...
(\Cref{app:commands}) repeatedly caught numbers that had silently gone out of date.
