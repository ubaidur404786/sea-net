...
The architecture proposed here is called \seanet{}, for \emph{Selective Evidence Aggregation
Network}, because it combines multi-scale temporal feature extraction with selective, class-specific
aggregation of the evidence found at each time step. It is introduced in this report and is not an
existing model. A complete \seanet{} model is always the pair
\emph{encoder $+$ pooling head}. Five \textbf{design goals}, fixed before implementation, motivate
the choices that follow. The encoder has to be \textbf{small}, with substantially fewer parameters
than InceptionTime and at a scale compatible with a microcontroller target. It has to be
\textbf{at least as accurate} as that baseline under an identical training configuration. It has to
be \textbf{length-preserving}, producing one embedding per time step at every stage, because the
interpretation has one value per time step and AOPCR masks individual time steps, so $\nsteps$ must
never change inside the encoder. It has to be \textbf{local}: each embedding must depend on a
bounded window of the input, otherwise the importance map becomes diffuse and uninformative.
Finally, the design has to be \textbf{modular}, with the encoder and the pooling head selectable
...
$\emb_j$ that \Cref{eq:encoder_generic} requires, which is the \textbf{length-preserving} goal. The
dilation grows geometrically from
...
bounded window, which is the \textbf{local} goal. Finally, encoder and head communicate through one
fixed contract: the
...
from configuration alone -- the \textbf{modular} goal.
...
\begin{figure}[t]
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=1.0\linewidth]{figures/fig04_seanet_architecture.pdf}}
...
\Cref{fig:arch_block} draws both parts of this. \Cref{eq:unit} examines the same signal at three
time scales at once: a kernel of size 5 covers a narrow local pattern, a kernel of size 23 reaches
about four times further, and their outputs are summed, so the block never has to commit to one
scale in advance. The pointwise convolution $P$ then combines what the three kernels found,
BatchNorm \citep{ioffe2015batchnorm} keeps activations on a stable scale, ReLU provides the
non-linearity and dropout regularises. The residual connection in \Cref{eq:block} makes the block an
identity plus a correction, which keeps deep stacks trainable. The lower panel of
\Cref{fig:arch_block} shows what the dilation buys: the same five taps of the $k=5$ filter span five
neighbouring time steps at $\delta = 1$ and 65 of them at $\delta = 16$, at no extra parameter
cost.
...
same step, roughly $29\times$ more. Two consequences follow: the \textbf{small} goal becomes
reachable at all, and
the pointwise convolution now dominates the parameter count, which is what \Cref{sec:var:bottleneck}
attacks.

% Drawn in TikZ in figures/fig_block_tikz.tex, switched by \showblockfigtrue /
% \showblockfigfalse in preamble.tex. The text above DOES cite it now, so switching it off
% would leave a dangling reference -- remove the two \Cref{fig:arch_block} first.
\ifshowblockfig
\begin{figure}[t]
  \centering
...
$(B, \dmodel, \nsteps)$, so the \textbf{length-preserving} and \textbf{modular} goals are
unaffected. This is the smallest encoder built here
and the one used in the headline result.

\begin{figure}[htbp]
  \centering
  \input{figures/fig_var_bottleneck}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \input{figures/fig_var_inputgate}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \input{figures/fig_var_spiketrend}
...
contradicting the \textbf{local} goal. \textbf{Pruning or quantising InceptionTime} would reduce
size but not address
...
(\Cref{sec:pool:classwise}--\Cref{sec:pool:attnmax}) follow directly from the three limitations of
conjunctive pooling identified in \Cref{sec:bg:millet}. The last two adapt mechanisms published for whole-slide image classification
...
The heads are presented in the order of the limitations they address (\Cref{sec:bg:millet}):
\Cref{sec:pool:classwise} takes on the shared attention gate, \Cref{sec:pool:softmax} the
unnormalised weights, and \Cref{sec:pool:adaptive}--\Cref{sec:pool:dsmil} the fixed mean, through
five different aggregation strategies.
...
\textbf{Limitation addressed: the shared attention gate.} The single gate of \Cref{eq:conj_gate}
must serve all classes at once.
...
\textbf{Limitation addressed: the unnormalised weights.} The sigmoid gate makes the bag score depend
...
\textbf{Limitation addressed: the fixed mean.} It is appropriate only when the evidence is spread
...
\textbf{Limitation addressed: the fixed mean, directly.} On a series of hundreds of time steps most
positions
...
\textbf{Limitation addressed: the fixed mean, as a control.} Adaptive class-wise pooling also
removes the fixed
...
\textbf{Limitation addressed: the shared gate and the fixed mean, through a stronger attention
branch.} The baseline gate is
...
\textbf{Limitation addressed: the fixed mean, when the evidence is scattered.} A single decisive
time step
