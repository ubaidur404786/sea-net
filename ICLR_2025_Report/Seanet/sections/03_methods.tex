...
\begin{enumerate}\itemsep1pt
  \item[\textbf{G1}] \textbf{Small.} Substantially fewer parameters than InceptionTime, at a scale
        compatible with a microcontroller target.
  \item[\textbf{G2}] \textbf{At least as accurate}, under an identical training configuration.
  \item[\textbf{G3}] \textbf{Length-preserving.} One embedding per time step at every stage: the
  ...
  \item[\textbf{G4}] \textbf{Local.} Each embedding must depend on a bounded window of the input,
        otherwise the importance map becomes diffuse and uninformative.
  \item[\textbf{G5}] \textbf{Modular.} Encoder and pooling head selectable from a configuration
        file, so any pair can be trained without modifying code.
\end{enumerate}
...
(slot~2). \Cref{fig:pool_template} draws that template and \Cref{fig:pool_aggregators} applies each
head's slot-2 operator to the same evidence profile.
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_pool_template}
  \caption{The template shared by every MIL pooling head. The embedding $\emb_j$ feeds an attention
  branch (\textbf{slot 1}) and a per-time-step classifier; their product is the evidence $g_j^k$,
  and an aggregator over time (\textbf{slot 2}) reduces the $\nsteps$ evidence values to the bag
  score. The interpretation consists of those same $g_j^k$ values rather than a second branch, which
  is why it cannot disagree with the prediction.}
  \label{fig:pool_template}
\end{figure}

\begin{figure}[htbp]
  \centering
  \input{figures/fig_pool_aggregators}
  \caption{What each pooling head does in slot~2, applied to the \emph{same} evidence profile
  $g_j^k$ so that the panels differ only by the aggregator. Conjunctive and class-wise pooling count
  every time step equally; softmax and adaptive pooling weight them smoothly; Top-$k$ keeps a few
  and discards the rest outright; attention-max and dual-stream concentrate on one decisive step.
  Only Top-$k$ makes the interpretation exactly zero where the model used no evidence, which is the
  mechanism behind its NDCG@$n$ advantage in \Cref{sec:results}.}
  \label{fig:pool_aggregators}
\end{figure}
