# 12 - Grid5000 (Lille + Sophia)

> **One thing to set before anything else.** The pipeline picks its device and its MLflow store
> from `configs/environments/<env>.yaml`. On the cluster that must be `grid5000`:
>
> ```bash
> export SEANET_ENV=grid5000        # `scripts/run_all.sh` does this for you
> ```
>
> or pass `--env grid5000` on every command. There is **no separate Grid5000 training script** -
> `scripts/run_all.sh` calls the same `python main.py train`, so the training and evaluation code
> is byte-identical on both machines and only the environment file differs.
...
open the web page. Full steps are in [09_mlflow.md](09_mlflow.md). Short version,
run on your **laptop**:
...
- **`CONFLICT (content): Merge conflict in <file>`** → both sides changed the same lines.
 ...
 seanet/seanet_spiketrend_topk seanet/seanet_bottleneck_topk seanet/seanet_gated_last_adaptive
 
  seanet/seanet_inputgate_adaptive seanet/seanet_spiketrend_adaptive                │
│      │ seanet/seanet_slim_topk

bash scripts/run_all.sh full

---

## Appendix: node reservation cheat-sheet (which `oarsub` gets which GPU)

```bash
# H100 94 GiB, 6 nodes x 2 GPUs - best availability
